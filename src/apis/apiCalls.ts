import { useMutation, useQuery, useQueryClient, UseQueryOptions, UseMutationResult } from "@tanstack/react-query";
...
export const useRegister = () => {
  return useMutation<RegisterResponse, AxiosError, RegisterData>({
    mutationFn: async (userData: RegisterData) => {
      const { data } = await http.post<RegisterResponse>(Endpoints.REGISTER, userData);
      return data;  
    },
  });
};
...
export const useCheckCredentials = () => {
  return useMutation<CheckCredentialsResponse, AxiosError, { email: string; phone: string; role: string }>({
    mutationFn: async (credentials: { email: string; phone: string; role: string }) => {
      const { data } = await http.post<CheckCredentialsResponse>(Endpoints.CHECK_CREDENTIALS, credentials);
      return data;
    },
  });
};
...
export const useUser = () => {
  return useQuery<UserDetailResponse, AxiosError>({
    queryKey: ["user"],
    queryFn: async () => {
      const { data } = await http.get<UserDetailResponse>(Endpoints.GET_MY_PROFILE_DETAILS);
      return data;
    },
  });
};
...
export const useSendOtp = () => {
  return useMutation<SendOtpResponse, AxiosError, SendOtpData>({
    mutationFn: async (otpData: SendOtpData) => {
        const { data } = await http.post<SendOtpResponse>(Endpoints.SEND_OTP, otpData);
      return data;
    },
  });
};
...
export const useVerifyOtp = () => {
  return useMutation<GeneralResponse, AxiosError, VerifyOtpData>({
    mutationFn: async (otpData: VerifyOtpData) => {
      const { data } = await http.post<GeneralResponse>(Endpoints.VERIFY_OTP, otpData);
      return data;
    },
  });
};
...
export const useServices = () => {
  ...
  const mutation = useMutation({
    mutationFn: async (newServices: Service[]) => {
      const { data } = await http.post<ServicesResponse>(Endpoints.ADD_NEW_SERVICE, newServices);
      return data.records;
    },
    onSuccess: () => {
  ...
  const updateService = (updatedService: Service) => {
    mutation.mutate(services.map(service => service.id === updatedService.id ? updatedService : service));
  };
...
> => {
  return useMutation({
    mutationFn: async (newService: Partial<ServiceResponse>) => {
      const response = await http.post<ServiceResponse>(Endpoints.ADD_NEW_SERVICE, newService);
      return response;
    },
  });
};
...
> => {
  return useMutation({
    mutationFn: async (data: Service) => {
      const response = await http.put<GeneralResponse>(Endpoints.EDIT_MY_SERVICES, data);
      return response;
    },
  });
};
...
export const useDeleteService = () => {
  return useMutation({
    mutationFn: async (id: number) => {
      const { data } = await http.delete(Endpoints.DELETE_MY_SERVICES, {params: {id}});
      return data;
    },
  });
};
...
export const useServicesByCategory = (
  ...
  return useQuery<CategoryServicesResponse, AxiosError>({
    ...
    queryFn: async () => {
      const { data } = await http.get<CategoryServicesResponse>(Endpoints.SERVICES_AGAINST_CATEGORY, {params: {categoryId}});
      return data;
    },
    ...options,
  });
...
export const useChangePassword = () => {
  ...
  return useMutation<GeneralResponse, AxiosError, ChangePasswordData>({
    mutationFn: async (passwordData: ChangePasswordData) => {
      ...
      const { data } = await http.put<GeneralResponse>(Endpoints.CHANGE_PASSWORD, passwordData);
      return data;
    },
  });
};
...
export const useUpdateProfile = () => {
  ...
  return useMutation<UserResponse, AxiosError, { formData: ProfileFormData; image?: File }>({
    mutationFn: async (formData: { formData: ProfileFormData; image?: File }) => {
      ...
      const { data } = await http.post<UserResponse>(Endpoints.ADD_BOOKMARK, formDataToSend, {
        headers: {
...
export const useRequestResetOtp = () => {
  return useMutation<GeneralResponse, AxiosError, { email: string }>({
    mutationFn: async (resetData: { email: string }) => {
      const { data } = await http.post<GeneralResponse>(Endpoints.SEND_OTP, resetData);
      return data;
    },
  });
};
...
export const useResetPassword = () => {
  return useMutation<GeneralResponse, AxiosError, ResetPasswordData>({
    mutationFn: async (resetData: ResetPasswordData) => {
        const { data } = await http.post<GeneralResponse>(Endpoints.RESET_PASSWORD, {
...
export const useCnicVerification = () => {
  return useMutation<CnicVerificationResponse, AxiosError, CnicVerificationRequest>({
    mutationFn: async (verificationData: CnicVerificationRequest) => {
      ...
      const { data } = await http.post<CnicVerificationResponse>(Endpoints.ADD_VERIFICATION_CNIC, formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
...
export const useLivePhotoVerification = () => {
  return useMutation<LivePhotoVerificationResponse, AxiosError, LivePhotoVerificationRequest>({
    mutationFn: async (verificationData: LivePhotoVerificationRequest) => {
      const formData = new FormData();
      formData.append("live_photo", verificationData.live_photo);

      const { data } = await http.post<LivePhotoVerificationResponse>(Endpoints.ADD_VERIFICATION_LIVE_PHOTO, formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
...
export const usePassportVerification = () => {
  return useMutation<PassportVerificationResponse, AxiosError, PassportVerificationRequest>({
    mutationFn: async (verificationData: PassportVerificationRequest) => {
      ...
      formData.append("passport_photo", verificationData.passport_photo);
        ...
        const { data } = await http.post<PassportVerificationResponse>(Endpoints.ADD_VERIFICATION_PASSPORT, formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
...
export const useVerificationCheck = () => {
  return useQuery<VerificationCheckResponse, AxiosError>({
    ...
    queryFn: async () => {
      const { data } = await http.get<VerificationCheckResponse>(Endpoints.CHECK_ACCOUNT_VERIFICATION);
      return data;
    },
  });
};
...
export const useWalletCredit = () => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await http.get<WalletCreditResponse>(Endpoints.SHOW_WALLET_CREDIT);
      return data;
    },
  });
};
...
export const useCustomerTransactions = () => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await http.get<TransactionResponse>(Endpoints.SHOW_CUSTOMER_TRANSACTION);
      return data;
    },
  });
};
...
export const useInitPaymentGateway = () => {
  return useMutation({
    mutationFn: async (paymentData: AddToWalletData) => {
      const { data } = await http.post<PaymentGatewayResponse>(Endpoints.INIT_PAYMENT_GATEWAY, paymentData);
      return data;
    },
  });
};
...
export const useCancelBid = () => {
  return useMutation<CancelBidResponse, AxiosError, CancelBidRequest>({
    mutationFn: async ({ bid_id, bid_canceled_reason }: CancelBidRequest) => {
      const { data } = await http.post<CancelBidResponse>(Endpoints.CANCELED_BID_CUSTOMER, {
        bid_id,
        bid_canceled_reason,
      });
      return data;
...
export const useBidResponses = (bid_id: number | null) => {
  return useQuery<OfferResponse, AxiosError>({
    ...
    queryFn: async () => {
      const { data } = await http.get<OfferResponse>(Endpoints.SHOW_BID_RESPONSE_CUSTOMER, {
        params: { bid_id }
      });
      return data;
    },
...
export const useBookings = (
  ...
  return useQuery<BookingsResponse, AxiosError>({
    ...
    queryFn: async (): Promise<BookingsResponse> => {
      ...
      const { data } = await http.get<BookingsResponse>(Endpoints.SHOW_ACTIVE_BOOKINGS, {
        params: { role },
      });
      return data;
    },
...
export const useBookingDetails = (
  ...
  return useQuery<BookingsResponse, AxiosError>({
    ...
    queryFn: async (): Promise<BookingsResponse> => {
      const { data } = await http.get<BookingsResponse>(Endpoints.GET_BOOKING_DETAILS, {
        params: { bookingId },
      });
      return data;
    },
...
export const useCancelBooking = () => {
  return useMutation<GeneralResponse, AxiosError, string>({
    mutationFn: async (bookingId: string) => {
      const { data } = await http.put<GeneralResponse>(Endpoints.SHOW_ALL_CANCELED_BOOKINGS, { bookingId });
      return data;
    },
  });
};
...
export const useCreateBid = () => {
  return useMutation<BidResponse, AxiosError, CreateBidData>({
    mutationFn: async (bookingData: CreateBidData) => {
      ...
      const { data } = await http.post<BidResponse>(Endpoints.BID_CREATION_CUSTOMER, formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
...
export const useDirectBooking = () => {
  return useMutation<CreateBookingResponse, AxiosError, CreateBookingData>({
    mutationFn: async (bookingData: CreateBookingData) => {
      ...
      const { data } = await http.post<CreateBookingResponse>(Endpoints.PLACE_ORDER, formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
...
export const useUpdateLocation = () => {
  ...
  >({
    mutationFn: async (locationData: { latitude: string; longitude: string }) => {
      ...
      const { data } = await http.patch<{ error: boolean; message: string }>(Endpoints.UPDATE_LOCATION, formData, {
        headers: {
...
export const useCategories = () => {
  ...
  const { data: categories = [], isLoading, error } = useQuery<Category[]>({
    queryKey: ['categories'],
    queryFn: async () => {
      const { data } = await http.get<CategoriesResponse>(Endpoints.GET_ALL_CATEGORIES);
      return data.records;
    },
  });
  ...
  const mutation = useMutation({
    mutationFn: async (newCategories: Category[]) => {
      const { data } = await http.post<CategoriesResponse>(Endpoints.GET_ALL_CATEGORIES, newCategories);
      return data.records;
    },
    onSuccess: () => {
  ...
  const updateCategory = (updatedCategory: Category) => {
    mutation.mutate(categories.map(category => category.id === updatedCategory.id ? updatedCategory : category));
  };
...
export const useCategory = (id: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await http.get<CategoryResponse>(Endpoints.GET_CATEGORY_DETAILS, {params: {id}});
      return data;
    },
  });
};
...
export const useSpecialOffers = () => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await http.get<SpecialOffersResponse>(Endpoints.SHOW_SPECIAL_OFFERS);
      return data.records;
    },
  });
};
...
export const useWorkerAvailability = (service_id: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await http.get(Endpoints.GET_WORKER_AVAILABILITY, {params: {service_id}});
      return data;
    },
  });
};
