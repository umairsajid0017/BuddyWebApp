...
export const chatService = {
  // Send a message
  async sendMessage(message: Omit<ChatMessageFirebase, "id" | "timestamp">) {
    try {
  ...
  subscribeToMessages(
    chatRoomId: string,
    callback: (messages: ChatMessageFirebase[]) => void,
  ) {
    ...
    return onSnapshot(q, (querySnapshot) => {
      const messages: ChatMessageFirebase[] = [];
      querySnapshot.forEach((doc) => {
        const data = doc.data();
        messages.push({
          id: doc.id,
